...
cdef extern void search_tree_float(tree_float *kdtree, float *pa, int8_t *code, float *point_coords, int8_t *query_code,
...
cdef extern void search_tree_double(tree_double *kdtree, double *pa, int8_t *code, double *point_coords, int8_t *query_code,
...
cdef class KDTree:
    ...
    def __init__(KDTree self, np.ndarray data_pts not None, np.ndarray code not None, int leafsize=16):
        ...
        cdef np.ndarray[int8_t, ndim=1] code_int8

        code_int8 = np.ascontiguousarray(code.ravel(), dtype=np.int8)
        self._data_code_int8 = <int8_t *> code_int8.data
        self.code = code_int8
    ...
    def query(KDTree self, np.ndarray query_pts not None, np.ndarray query_code not None,
        ...
        query_code : numpy array
            Query codes with shape (m, dims)
        alpha : float
            The hyperparameter to control knn, if alpha = 0 will degrade to pure knn, if alpha < 0 will ignore L2 distance
        scaler : float
            The scaler to scaling the L1 distance to L2 distance
        k : int
        ...
        query_code_int8 = np.ascontiguousarray(query_code.ravel(), dtype=np.int8)
        query_code_data_int8 = <int8_t *> query_code_int8.data
        ...
        if self.data_pts.dtype == np.float32:
            with nogil:
                search_tree_float(self._kdtree_float, self._data_pts_data_float, self._data_code_int8,
                                  query_array_data_float, query_code_data_int8, num_qpoints, num_n, alpha_float, scaler_float, code_dims,
                                  dub_float, epsilon_float, query_mask_data, closest_idxs_data, closest_dists_data_float)

        else:
            with nogil:
                search_tree_double(self._kdtree_double, self._data_pts_data_double, self._data_code_int8,
                                   query_array_data_double, query_code_data_int8, num_qpoints, num_n, alpha_float, scaler_float, code_dims,
        ...
        return closest_dists_res, closest_idxs_res.astype(np.int32)
